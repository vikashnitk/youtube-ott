...
class ContinueWatchingViewHistory(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, db_column='user_id')  # Reference the User model
    movie = models.ForeignKey(Movie, null=True, blank=True, on_delete=models.CASCADE, db_column='movie_id')  # Optional ForeignKey to Movie
    episode = models.ForeignKey(Episode, null=True, blank=True, on_delete=models.CASCADE, db_column='episode_id')  # Optional ForeignKey to Episode
